...
class tmhtmlcontent2 extends Module {
	...
	public function __construct() {
		$this->name = 'tmhtmlcontent2';    // Defines module name
		...
		$this->displayName = $this->l('TM HTML content 2');
	...
	private function installDB() {
		...
		Db::getInstance()->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'tmhtmlcontent2`');

		if (!Db::getInstance()->Execute('
			CREATE TABLE `'._DB_PREFIX_.'tmhtmlcontent2` (
	...
	public function uninstall() {
		...
		$images = Db::getInstance()->ExecuteS('SELECT image FROM `'._DB_PREFIX_.'tmhtmlcontent2`');
		foreach ($images as $image)
			$this->_deleteImages($image);
		
	
		if (!Db::getInstance()->Execute('DROP TABLE IF EXISTS `'._DB_PREFIX_.'tmhtmlcontent2`') OR
			!parent::uninstall())
			return false;
		return true;	
	}
	...
	private function _displayForm() {	
		...
		foreach ($this->_languages as $language) {
			...
			foreach ($hooks[$language['id_lang']] as $hook) {
				$items[$language['id_lang']][$hook] =  Db::getInstance()->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'tmhtmlcontent2` WHERE id_shop = '.$id_shop.' AND id_lang = '.$language['id_lang'].' AND hook = \''.$hook.'\' ORDER BY item_order ASC');
			}
		}
	...
	private function _addItem() {
		$id_shop = (int)$this->context->shop->id;
		$lastOrder = Db::getInstance()->ExecuteS('SELECT item_order FROM `'._DB_PREFIX_.'tmhtmlcontent2` WHERE id_shop = '.$id_shop.' AND id_lang = '.(int)Tools::getValue('lang_id').' AND hook = \''.Tools::getValue('item_hook').'\' ORDER BY item_order DESC LIMIT 1');
		...
		$insert = Db::getInstance()->Execute('
			INSERT INTO `'._DB_PREFIX_.'tmhtmlcontent2` ( 
	...
	private function _updateItem() {
		...
		if(!empty($_FILES['item_img']['name'])){
			if ($oldImage = Db::getInstance()->ExecuteS('SELECT image FROM `'._DB_PREFIX_.'tmhtmlcontent2` WHERE id_item = '.(int)Tools::getValue('item_id')))
		...
		$update = Db::getInstance()->Execute('
			UPDATE `'._DB_PREFIX_.'tmhtmlcontent2` SET 
		...
		if(!$update){
			if ($newImage = Db::getInstance()->ExecuteS('SELECT image FROM `'._DB_PREFIX_.'tmhtmlcontent2` WHERE id_item = '.(int)Tools::getValue('item_id')))
				$this->_deleteImages($oldImage[0]);
			$this->context->smarty->assign('error', $this->l('An error occured while saving data.'));	
			return false;			
		}
	...
	public function _removeItem() {
		$id_shop = (int)$this->context->shop->id;
		if ($delImage = Db::getInstance()->ExecuteS('SELECT image FROM `'._DB_PREFIX_.'tmhtmlcontent2` WHERE id_item = '.(int)Tools::getValue('item_id')))
			$this->_deleteImages($delImage[0]);
			
		Db::getInstance()->delete(_DB_PREFIX_.'tmhtmlcontent2', 'id_item = '.(int)Tools::getValue('item_id'));
	
		if(Db::getInstance()->Affected_Rows() == 1){
			Db::getInstance()->Execute('
				UPDATE `'._DB_PREFIX_.'tmhtmlcontent2` 
	...
	public function hookDisplayTop() {
		...
		$hook_name = 'top';
		$items = array();
		$items = Db::getInstance()->ExecuteS('SELECT * FROM `'._DB_PREFIX_.'tmhtmlcontent2` WHERE id_shop = '.$id_shop.' AND id_lang = '.$id_lang.' AND hook = \''.$hook_name.'\' AND active = 1 ORDER BY item_order ASC');
